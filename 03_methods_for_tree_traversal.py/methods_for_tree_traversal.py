...
class BST:
    ...
    def WideAllNodes(self):
        ...
        return self.WideAllNodes_recursion(current_level_nodes, result_nodes)

    def WideAllNodes_recursion(self, current_level_nodes, result_nodes):       
        result_nodes = result_nodes + current_level_nodes        
        new_level_nodes = []
        for current_node in current_level_nodes:
            if current_node.LeftChild != None:
                new_level_nodes.append(current_node.LeftChild)
            if current_node.RightChild != None:
                new_level_nodes.append(current_node.RightChild)
        if new_level_nodes == []:
            return result_nodes
        return self.WideAllNodes_recursion(tuple(new_level_nodes), result_nodes)
    ...
    def DeepAllNodes(self, order):
        ...
        if self.Root == None:
            return nodes_tuple
        current_node = self.Root
        return self.DeepAllNodes_recursion(current_node, nodes_tuple, order)

    def DeepAllNodes_recursion(self, current_node, nodes_tuple, order):
        if (    current_node.LeftChild == None
            and current_node.RightChild == None
           ):
            return nodes_tuple + (current_node,)
        if order == 2:
            nodes_tuple = nodes_tuple + (current_node,)
        if current_node.LeftChild != None:
            nodes_tuple = self.DeepAllNodes_recursion(current_node.LeftChild, nodes_tuple, order)
        if order == 0:
            nodes_tuple = nodes_tuple + (current_node,)
        if current_node.RightChild != None:
            nodes_tuple = self.DeepAllNodes_recursion(current_node.RightChild, nodes_tuple, order)
        if order == 1:
            nodes_tuple = nodes_tuple + (current_node,)
        return nodes_tuple
    ...
    def InvertTree(self):
        ...
        if self.Root == None:
            return
        current_node = self.Root
        return self.InvertTree_recursion(current_node)

    def InvertTree_recursion(self, current_node):
        if (    current_node.LeftChild == None
            and current_node.RightChild == None
           ):
            return
        ...
        if current_node.LeftChild != None:
            self.InvertTree_recursion(current_node.LeftChild)
            new_left_child = current_node.LeftChild
        if current_node.RightChild != None:
            self.InvertTree_recursion(current_node.RightChild)
            new_right_child = current_node.RightChild
        current_node.LeftChild = new_right_child
        current_node.RightChild = new_left_child
